...
if(request.getParameter("incluir")!=null){
	...
	//Recupera os valores trazidos do formulário e atribui ao objeto servicometroquadrado
	servicometroquadrado.servico.servicoID	= Integer.parseInt(request.getParameter("servicoID"));
	servicometroquadrado.servicos.servicoID = Integer.parseInt(request.getParameter("servID"));
	servicometroquadrado.valor 				= Float.parseFloat(request.getParameter("total"));
	servicometroquadrado.descricao			= request.getParameter("servicos");
	servicometroquadrado.quantidade			= Float.parseFloat(request.getParameter("qtdMetros").replace(",","."));
	//Atribui o valor ao objeto servico
	servico.servicoID				= servicometroquadrado.servico.servicoID;
	
	
	//Insere o produto na base de dados
	st.execute(servicometroquadrado.salvaServico());
	...
	servico.valor = (valorAtual + servicometroquadrado.valor);
...
rs02 = st02.executeQuery(servicos.servicosM());
%>
...
if(pesquisou){
	if(rs03.next()){
		...
		servicoValor = rs03.getString("valorM");
...
function bucarPorServico(){
	...
	if(document.form1.servicosID.value == ""){
		...
		window.location.href="sis_add_servicoM.jsp?servicosID="+servicosID+"&buscarPorServicoM=ok&servicoID="+servicoID;
	}
}
...
function metragem(){
	...
	var valorMetro = parseFloat(document.form1.valor.value);
	var qtdMetros = document.form1.qtdMetros.value;
	
	qtdMetros = parseFloat(qtdMetros.replace(",","."));
	
	
	
	document.form1.total.value = (valorMetro * qtdMetros).toFixed(2);
...
<form name="form1" method="get" action="sis_add_servicoM.jsp" onSubmit="return verForm(this)" >
<table width="490" border="0" align="center" cellpadding="2" cellspacing="2">
 ...
 <tr>
   ...
   <select name="servicosID" style="width:265px;">
    ...
    <%while(rs02.next()){ %>
      <%if(rs02.getString("tipo").equals("V")){%>
   	   <option value="<%=rs02.getInt("servicoID") %>">VB - <%=rs02.getString("servicoTitulo") %></option>
      <%}else{%>
 ...
 <tr>
  <td width="107" align="left">Valor por m&sup2;</td>
 ...
 <tr>
  <td width="107" align="left">Metragem</td>
  <td colspan="2" align="left">
  <input name="qtdMetros" type="text" id="qtdMetros" onBlur="metragem()" value="0" onKeyPress="return numero(this.value)" size="20" maxlength="10" >
  &nbsp;
<input type="button" name="calcularMetragem" onClick="metragem()" value="Calcular Metragem">
  </td>
 </tr>
 <tr>
  <td width="107" align="left">Valor TOTAL</td>
  <td colspan="2" align="left"><input name="total" type="text" onBlur="metragem()" value="0.00" size="20" maxlength="10" >
  </td>
 </tr>
 <tr>
