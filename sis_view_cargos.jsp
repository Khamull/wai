...
if(request.getParameter("nome") != null){
	rs = st.executeQuery(cargo.pesquisaCargo(request.getParameter("nome")));
}else{
...
if(request.getParameter("acao") != null){
	...
	st.execute(cargo.statusCargo(numeroAcao));
	response.sendRedirect("sis_view_cargos.jsp");
}
%>
...
<script type="text/javascript">
 function excluir(cargoID){
	if(confirm("Tem certeza que deseja Excluir esse cargo?")){
		window.location.href="sis_delete_manager_cargo.jsp?cargoID="+cargoID;
	}
 }
</script>
...
<tr>
 ...
 <br />
  <table width="470" align="center" cellpadding="0" cellspacing="0">
    ...
    <form name="formBusca" method="post">
     <table width="335" style="border:1px solid #333; height:30px" align="left" cellpadding="0" cellspacing="0">
      <tr>
       ...
       <input type="text" name="nome" maxlength="100" size="50" style="background-image:url(images/buscacargo.png); height:18px; border:none"  onclick="this.style.backgroundImage = 'none'" onblur="if(this.value == '')this.style.backgroundImage = 'url(images/buscacargo.png)'"/>
       </td>
   ...
   <tr bgcolor="#EEEEEE">
    <td width="250" align="left"><strong>Titulo</strong></td>
...
%>
   ...
   <tr>
    <td height="30" width="250" align="left"><%=rs.getString("cargoTitulo")%></td>
    <td height="30" width="60" align="center"><a href="sis_update_cargo.jsp?cargoID=<%=rs.getString("cargoID")%>"><img src="ico/ico_editar.png" width="20" height="20" border="0" title="Editar <%=rs.getString("cargoTitulo")%>" /></a></td>
    <%
	//Verifica o status se está ativo ou não
	String status = rs.getString("cargoAtivo");
	if(status.equals("S")){
	%>
    <td height="30" width="60" align="center"><a href="sis_view_cargos.jsp?acao=1&cargoID=<%=rs.getString("cargoID")%>"><img src="ico/ico_ativo.png" width="20" height="20" border="0" title="Desativar <%=rs.getString("cargoTitulo")%>" /></a></td>
    <%}else if(status.equals("N")){%>
    <td height="30" width="60" align="center"><a href="sis_view_cargos.jsp?acao=2&cargoID=<%=rs.getString("cargoID")%>"><img src="ico/ico_inativo.png" width="20" height="20" border="0" title="Ativar <%=rs.getString("cargoTitulo")%>" /></a></td>
    <%}%>
    
    <td height="30" width="60" align="center"><a href="javascript:excluir(<%=rs.getString("cargoID")%>)"><img src="ico/ico_excluir.png" width="20" height="20" border="0" title="Excluir <%=rs.getString("cargoTitulo")%>" /></a></td>
   </tr>
   <tr>
